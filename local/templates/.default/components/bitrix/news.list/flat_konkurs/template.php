...
<?endif;?>
<div class="row consurs__wrapper">
<?foreach($arResult["ITEMS"] as $arItem):?>
	<?
	$this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_EDIT"));
	$this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_DELETE"), array("CONFIRM" => GetMessage('CT_BNL_ELEMENT_DELETE_CONFIRM')));
	?>
	<div class="bx-newslist-container col-sm-6 col-md-4 consurs__item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
	<div class="bx-newslist-block consurs__video">
		<?
			if ($arItem["PROPERTIES"]["VIDEO_CODE"]["VALUE"]):?>
				<div class="bx-newslist-img">
					<a href="https://youtu.be/<?=$arItem["PROPERTIES"]["VIDEO_CODE"]["VALUE"];?>" class="consurs_video" data-fancybox="videoslide" target="_blank">
            <img class="card-img-top img-fluid" src="https://img.youtube.com/vi/<?=$arItem["PROPERTIES"]["VIDEO_CODE"]["VALUE"];?>/mqdefault.jpg">
        </a>
		...
		<?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
			<h3 class="bx-newslist-title concurs__name">
				<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
		...
		<?foreach($arItem["FIELDS"] as $code=>$value):?>
			...
			<?elseif ($value != ""):?>
				<div class="bx-newslist-other concurs__row"><i class="fa"></i> <?=GetMessage("IBLOCK_FIELD_".$code)?>:
					<?=$value;?>
				</div>
			<?endif;?>
		<?endforeach;?>
		<?foreach($arItem["DISPLAY_PROPERTIES"] as $pid=>$arProperty):?>
			...
			<?elseif ($value != ""):?>
				<div class="bx-newslist-other concurs__row"><i class="fa"></i> <?=$arProperty["NAME"]?>:
					<span><?=$value;?></span>
				</div>
			<?endif;?>
		<?endforeach;?>
